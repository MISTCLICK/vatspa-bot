...
module.exports = class MuteCommand extends discord_js_commando_1.default.Command {
    ...
    async run(message, args) {
        ...
        if (durationRaw.endsWith('d')) {
            ...
            return message.reply('You can assign the duration using such units as:\n1m/1h/1d');
        }
        ...
        if (previousMutes === null) {
            ...
            target.send(`You were muted on **${message.guild}** by a moderator ${message.author} till ${moment_1.default(duration).utc().format('DD.MM.YYYY | HH:mm:ss')}!\n\nReason: ${reason}`);
        }
        else {
            let current = false;
