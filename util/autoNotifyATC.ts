  ...
  setInterval(async () => {
    ...
    if (prevStations && prevStations.result.controllers.length !== onlineStations.data.controllers.length) {
      ...
      for (const station of onlineStations.data.controllers) {
        if (!station.callsign.endsWith('ATIS') && !station.callsign.endsWith('OBS') && station.callsign.startsWith('EVRA')) currentATClist.push(station.callsign);
      }
      for (const station of prevStations.result.controllers) {
        if (!station.callsign.endsWith('ATIS') && !station.callsign.endsWith('OBS') && station.callsign.startsWith('EVRA')) oldATClist.push(station.callsign);
      }
